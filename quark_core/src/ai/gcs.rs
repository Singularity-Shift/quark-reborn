...
// `cloud_storage::Client` doesn't implement `Clone`, so we recreate it in `clone`.
impl Clone for GcsImageUploader {
    fn clone(&self) -> Self {
        Self {
...
mod tests {
    ...
    async fn test_gcs_uploader_creation() {
        ...
        let mock_credentials_base64 =
            general_purpose::STANDARD.encode(mock_credentials_json.as_bytes());

        // This will fail without real credentials, but tests the structure
        let result =
            GcsImageUploader::new(&mock_credentials_base64, "test-bucket".to_string()).await;
