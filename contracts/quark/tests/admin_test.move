...
module quark_test::admin_test {
    ...
    fun test_set_pending_admin(quark_test: &signer, admin: &signer) {
        ...
        assert!(admin_v3::is_admin(signer::address_of(quark_test)), EIS_NOT_ADMIN);
        admin_v3::set_pending_admin(quark_test, signer::address_of(admin));
        assert!(admin_v3::is_pending_admin(signer::address_of(admin)), EIS_NOT_PENDING_ADMIN);
    }
