...
module quark_test::admin_test {
    ...
    fun test_set_pending_admin(quark_test: &signer, admin: &signer) {
        ...
        assert!(admin_v5::is_admin(signer::address_of(quark_test)), EIS_NOT_ADMIN);
        admin_v5::set_pending_admin(quark_test, signer::address_of(admin));
        assert!(admin_v5::is_pending_admin(signer::address_of(admin)), EIS_NOT_PENDING_ADMIN);
    }
    ...
    fun test_set_reviewer_pending_admin(quark_test: &signer, reviewer: &signer) {
        ...
        assert!(admin_v5::is_reviewer_pending_admin(signer::address_of(reviewer)), EIS_NOT_REVIEWER_PENDING_ADMIN);
    }
    ...
    #[expected_failure(abort_code = 2, location = admin_v5)]
    fun test_should_not_pending_admin_accept_admin(quark_test: &signer, admin: &signer, user: &signer) {
        init_module(quark_test);
        admin_v5::set_pending_admin(quark_test, signer::address_of(admin));
        admin_v5::accept_admin(user);
    }

    #[test(quark_test = @quark_test, admin = @0x2, user = @0x3)]
    #[expected_failure(abort_code = 1, location = admin_v5)]
    fun test_should_not_admin_set_pending_admin(quark_test: &signer, admin: &signer, user: &signer) {
        init_module(quark_test);
        admin_v5::set_pending_admin(admin, signer::address_of(user));
    }
}