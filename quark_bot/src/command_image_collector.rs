...
impl CommandImageCollector {
    ...
    pub async fn add_command(self: Arc<Self>, msg: Message) {
        // Cancel any existing pending command for this user/chat
        let key = (msg.chat.id, msg.from.as_ref().map(|u| u.id.0 as i64).unwrap_or(0));
    ...
    pub async fn try_attach_photo(self: Arc<Self>, msg: Message) {
        let user_id = msg.from.as_ref().map(|u| u.id.0 as i64).unwrap_or(0);
        let key = (msg.chat.id, user_id);
        if let Some(mut entry) = self.pendings.get_mut(&key) {
            // Attach photo
            entry.extra_photos.push(msg);
            // restart debounce
            self.reset_timer(key);
        }
    }

    fn reset_timer(self: &Arc<Self>, key: (ChatId, i64)) {
    ...
    async fn finalize(self: &Arc<Self>, key: (ChatId, i64)) {
        if let Some((_k, pending)) = self.pendings.remove(&key) {
            ...
            if all_msgs.len() == 1 {
                ...
                if let Err(e) = commands::handle_chat(
                    self.bot.clone(),
                    msg,
                    self.db.clone(),
                ...
                if let Err(e) = commands::handle_grouped_chat(
                    self.bot.clone(),
                    all_msgs,
                    self.db.clone(),
                    self.openai_api_key.clone(),
