...
use crate::{ai::moderation::dto::{ModerationOverrides, ModerationState}, dependencies::BotDependencies, payment::dto::PaymentPrefs, utils::create_purchase_request};
    ...
    let sentinel_on = bot_deps.sentinel.get_sentinel(chat_id.clone());
        if sentinel_on {
            ...
            if let Some(_) = &msg.from {
                let moderation_state = bot_deps.moderation.get_moderation_state(chat_id.clone()).unwrap_or(ModerationState {
