    ...
    pub async fn generate_response(
        ...
        if !image_urls.is_empty() {
            ...
            if !input.trim().is_empty() {
                let user_context = if !cached_images.is_empty()
                    && user_uploaded_image_urls.is_empty()
                {
                    // Only cached images present = previously generated by AI
                    format!(
                        "CONTEXT: The following image(s) were generated by you in a previous response. They may wish to discuss the image in detail, or may simply be acknowledging it. Use your best judgment based on their message to determine the appropriate level of analysis and response. The user is now saying: '{}'.",
                        input
                    )
                } else {
